...
PIPELINE_VERSION = "proto-image-v1"
...
def _compose(image: Image.Image, options: ProcessOptions) -> Image.Image:
    box = _subject_box(image, options.remove_background) if options.crop else (0, 0, image.width, image.height)
    if not box:
