...
class HttpAdminAdapter:
    ...
    def claim(self) -> RemoteJob | None:
        raw = self._admin_request("/api/image-processing-worker", {"action": "claim", "workerId": self.settings.worker_id, "capabilities": {"formats": ["jpeg", "png", "webp"], "outputs": ["jpeg"], "pipeline": "proto-image-v1"}})
    ...
    def complete(self, job: RemoteJob, payload: dict) -> None:
        self._admin_request("/api/image-processing-worker", {"action": "callback", "jobId": job.admin_job_id, "imageId": job.image_id, "claimId": job.claim_id, "outputPath": job.output_path, "workerId": self.settings.worker_id, "model": "rembg-local", "costUsd": 0, "processingMs": payload.get("processing_ms", 0), "quality": payload.get("quality") or {}, "warnings": payload.get("warnings") or []})
