...
export function canonicalPublishValues({ product, existing, submitted = {} }) {
  const canonicalPrice = finite(product?.sell_price);
  if (product && (canonicalPrice == null || canonicalPrice <= 0)) {
    return {
      blocked: true,
  ...
  if (product) {
    return {
      ...
      stockQty: finite(product.stock_qty) ?? 0,
      availableStock: finite(product.available_stock) ?? finite(product.stock_qty) ?? 0,
      corrections: evaluateCatalogueValues({
        websitePrice: submitted.price,
        canonicalPrice,
        websiteAvailable: submitted.availableStock,
        canonicalAvailable: product.available_stock ?? product.stock_qty,
