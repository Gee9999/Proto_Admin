...
export function saveArchivePrioritySkus(values) {
  ...
  try {
    sessionStorage.setItem(ARCHIVE_PRIORITY_STORAGE_KEY, JSON.stringify(skus));
...
export function readArchivePrioritySkus() {
  try {
    return normalizeArchivePrioritySkus(JSON.parse(sessionStorage.getItem(ARCHIVE_PRIORITY_STORAGE_KEY) || '[]'));
  } catch {
    return [];
  }
}
