...
export const ARRIVED_STOCK_STATUSES = Object.freeze([
  'landed_awaiting_grv',
  'partially_received',
]);
...
export function isStockHasArrived(value = {}) {
  const next = normalizeIncomingAvailability(value || {});
  return next.incomingQty > 0 && ARRIVED_STOCK_STATUSES.includes(next.incomingStatus);
}
...
export function resolveProductAvailability({ stockQty = null, toOrder = false, incoming = null } = {}) {
  ...
  if (isStockHasArrived(next)) {
    return {
      state: 'landed',
      label: 'Stock has arrived',
      guidance: 'Stock has arrived at our warehouse and is being prepared for sale. You can add it to your order now; final availability will be confirmed on your quotation.',
      canOrder: true,
      stockQty: stock,
      ...publicIncoming,
    };
