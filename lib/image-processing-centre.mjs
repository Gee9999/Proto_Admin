...
export const ALLOWED_TARGET_TABLES = Object.freeze(['website_stock', 'archived_products']);
...
export function normalizeCreateImage(raw = {}) {
  ...
  const targetTable = cleanText(raw.targetTable || 'website_stock', 40).toLowerCase();
...
export function deriveJobStatus(images = []) {
  ...
  if (statuses.every((s) => ['approved', 'published', 'restored'].includes(s))) return 'complete';
